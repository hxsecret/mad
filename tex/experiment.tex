...
\begin{table}[h!]
    \scriptsize
    \centering
    \caption{Comprehensive detection performance of MAD and baselines across both datasets. MalHug metrics are computed on 78 malicious and 11 benign models; Synthetic Corpus metrics are computed on 80 advanced malicious models with false positives evaluated on 40 benign TensorFlow-API models.}
    ...
    \SetCell[r = 1, c = 1]{c} \textbf{Method} & \SetCell[r = 1, c = 3]{c} \textbf{MalHug Dataset} & & & \SetCell[r = 1, c = 3]{c} \textbf{Synthetic Corpus} & & \\
...
\label{sec:rq3}
\subsection{RQ3: Efficiency and Deployability}
...
\textbf{Performance Overhead Analysis.} The performance impact of MAD on normal model execution is presented in Table~\ref{tab:overhead_result}. The results demonstrate that MAD introduces negligible overhead to the target system. Specifically, the total execution time increases by an average of only 5.69\%, with the overhead consistently remaining within a narrow 4.9\% to 6.0\% margin across fundamentally different model architectures.
...
To further demonstrate that our detection system minimally impacts inference efficiency, we conducted an in-depth analysis of the relationship between the number of inference iterations and the relative latency overhead. Figure~\ref{fig:inference_overhead} illustrates the execution time of the BERT model over 1 to 10 inference steps, comparing the vanilla execution with the MAD-monitored execution. As observed, the relative overhead percentage is highest during the initial executions but quickly drops and stabilizes at a lower baseline as the number of inferences increases. This phenomenon is highly consistent with framework behaviors: model loading and initialization phases trigger a dense burst of system calls such as extensive file I/O for reading model weights and `mmap` for memory mapping, which inherently introduces brief CPU-bound monitoring overhead. However, once the model enters the continuous inference phase, the intensive computations are primarily offloaded to hardware accelerators. These computations generate very few new system calls and do not occupy main CPU resources. Consequently, the dynamic instrumentation and system call monitoring introduce almost zero additional latency, allowing the initial loading overhead to be rapidly amortized over prolonged inference sessions.
...
While dynamic reasoning inherently introduces some delay compared to simple signature matching, this two-stage design keeps the overall overhead well within the acceptable budget for offline model vetting pipelines and pre-deployment security checks. Our experiments suggest that MAD can be seamlessly integrated as a final security gate in model release workflows without significantly delaying deployment operations.
...
\begin{table}[h!]
    ...
    \SetCell[r = 1, c = 1]{c} \textbf{MAD-S} & - & - & - & 30.3126 & 867075 \\
    \SetCell[r = 1, c = 1]{c} \textbf{MAD-L} & 85.25\%   & \textbf{98.73\%} & 91.5\% & \textbf{8.9778} & \textbf{50005} \\ \hline
    \SetCell[r = 1, c = 1]{c} \textbf{MAD} & \textbf{100.00\%} & \textbf{98.73\%} & \textbf{99.36\%} & 9.7934 & 57207.1 \\
    \end{tblr} }
