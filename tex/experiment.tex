...
\begin{itemize}
    ...
    \item \emph{Advanced TensorAbuse Synthetic Dataset.} To address the limitations of the MalHug Dataset and evaluate our system against highly stealthy, framework-native attacks, we constructed a second dataset using the TensorAbuse framework~\cite{zhu2025}. This corpus comprises two complementary subsets. The \textbf{malicious subset} contains 80 models: for each of 10 diverse foundation models, we embed 8 attack variants spanning four categories—File Exfiltration, IP Exposure, Malicious Code Insertion, and Remote Shell Acquisition—using TensorAbuse primitives. The \textbf{Benign Subset} contains 40 models that invoke the same high-risk native TensorFlow APIs for legitimate purposes such as loading dataset configurations or outputting debug tags, constructed to precisely quantify the false-positive rate.
\end{itemize}
...
\begin{table}[t!]
    \scriptsize
    \centering
    \caption{Comprehensive detection performance of \toolname{} and baselines across both datasets. }
...
\begin{table}[t!]
    \scriptsize
    \centering
    \caption{Number of detected malicious models by \toolname{} and baselines on the Advanced TensorAbuse Synthetic Dataset, categorized by attack type (20 models per type).}
    \resizebox{0.49\textwidth}{!}{\begin{tblr}{
        cell{1-2}{1-6} = {c},
        cell{3-7}{2-6} = {c},
        vline{2},
